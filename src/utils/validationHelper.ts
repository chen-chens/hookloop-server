...
const valExist = (req: Request, field: string, fieldName: string): ValReturn => {
...
const valLengthInRange = (req: Request, field: string, fieldName: string, min: number, max: number): ValReturn => {
  const fieldValue = req.body[field];
  if (fieldValue && !validator.isLength(fieldValue, { min, max })) {
...
const valMaxLength = (req: Request, field: string, fieldName: string, max: number): ValReturn => {
  const fieldValue = req.body[field];
  if (fieldValue && !validator.isLength(fieldValue, { max })) {
...
const valEnum = (req: Request, field: string, fieldName: string, enumArray: string[]): ValReturn => {
  ...
  if (fieldValue && !validator.isIn(fieldValue.enumArray, enumArray)) {
...
const getValidators = (req: Request, field: string, fieldName: string, rules: ValRule[]): ValReturn[] => {
  return rules.map((rule) => {
    switch (rule.type) {
      case "exist":
        return valExist(req, field, fieldName);
      case "paramId":
  ...
  return validErrors.length !== 0
    ? validErrors.reduce(
        (errorData, validationResult) => {
          return {
            field: `${errorData.field} ${validationResult.field}`,
